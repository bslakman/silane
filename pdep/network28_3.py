...
species(
    ...
    E0 = (772.241,'kJ/mol'),
    modes = [
        HarmonicOscillator(frequencies=([377.054,377.252,377.264,377.339,377.402,377.435,377.442,377.482],'cm^-1')),
        HinderedRotor(inertia=(0.00118226,'amu*angstrom^2'), symmetry=1, barrier=(0.119627,'kJ/mol'), semiclassical=False),
    ],
    spinMultiplicity = 3,
    opticalIsomers = 1,
    molecularWeight = (86.2724,'amu'),
    collisionModel = TransportData(shapeIndex=2, epsilon=(1971.36,'J/mol'), sigma=(5.118e-10,'m'), dipoleMoment=(0,'C*m'), polarizability=(0,'angstroms^3'), rotrelaxcollnum=0, comment="""Epsilon & sigma estimated with fixed Lennard Jones Parameters. This is the fallback method! Try improving transport databases!"""),
    energyTransferModel = SingleExponentialDown(alpha0=(3.5886,'kJ/mol'), T0=(300,'K'), n=0.85),
    thermo = NASA(polynomials=[NASAPolynomial(coeffs=[2.3996,0.0416055,-7.82068e-05,7.42926e-08,-2.63257e-11,92930.6,32.6613], Tmin=(100,'K'), Tmax=(879.485,'K')), NASAPolynomial(coeffs=[4.87327,0.0153249,-7.75001e-06,1.45365e-09,-9.72631e-14,93076.8,24.3495], Tmin=(879.485,'K'), Tmax=(5000,'K'))], Tmin=(100,'K'), Tmax=(5000,'K'), E0=(772.241,'kJ/mol'), comment="""Thermo group additivity estimation: group(Sis-H3si) + other(R) + group(si) + other(R) + group(si) + other(R) + radical(SisJ_SiLP) + radical(SiJ_LP_Si)"""),
)

species(
    label = '[Si][SiH2][Si](58)',
    structure = SMILES('[Si][SiH2][Si]'),
    E0 = (776.42,'kJ/mol'),
    modes = [
        HarmonicOscillator(frequencies=([180,777.18,777.21,777.212,777.212,777.23,777.232,777.233,777.254],'cm^-1')),
    ],
    spinMultiplicity = 3,
    opticalIsomers = 1,
    molecularWeight = (86.2724,'amu'),
    collisionModel = TransportData(shapeIndex=2, epsilon=(1971.36,'J/mol'), sigma=(5.118e-10,'m'), dipoleMoment=(0,'C*m'), polarizability=(0,'angstroms^3'), rotrelaxcollnum=0, comment="""Epsilon & sigma estimated with fixed Lennard Jones Parameters. This is the fallback method! Try improving transport databases!"""),
    energyTransferModel = SingleExponentialDown(alpha0=(3.5886,'kJ/mol'), T0=(300,'K'), n=0.85),
    thermo = NASA(polynomials=[NASAPolynomial(coeffs=[2.97956,0.0248925,-3.52939e-05,3.15096e-08,-1.15621e-11,93416.1,29.2059], Tmin=(100,'K'), Tmax=(795.165,'K')), NASAPolynomial(coeffs=[4.40744,0.013738,-6.75978e-06,1.30501e-09,-9.08635e-14,93314.6,23.434], Tmin=(795.165,'K'), Tmax=(5000,'K'))], Tmin=(100,'K'), Tmax=(5000,'K'), E0=(776.42,'kJ/mol'), comment="""Thermo group additivity estimation: group(Sis-H3si) + other(R) + group(si) + other(R) + group(si) + other(R) + radical(SiJ_LP_Si) + radical(SiJ_LP_Si)"""),
)
...
reaction(
    label = 'reaction12',
    reactants = ['[Si][SiH2](56)', 'SiH(11)'],
    products = ['[Si][SiH2][SiH](40)'],
...
reaction(
    label = 'reaction14',
    reactants = ['[Si][SiH2][Si](58)', '[H](16)'],
    products = ['[Si][SiH2][SiH](40)'],
    transitionState = 'TS14',
    kinetics = Arrhenius(A=(1.44114e+07,'m^3/(mol*s)'), n=0.100587, Ea=(0,'kJ/mol'), T0=(1,'K'), comment="""Estimated using template (Y_rad;Y_rad) for rate rule (H_rad;Si_rad)
Multiplied by reaction path degeneracy 2
Ea raised from -0.8 to 0 kJ/mol."""),
)
