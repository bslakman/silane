species(
    ...
    E0 = (205.861,'kJ/mol'),
    modes = [
        HarmonicOscillator(frequencies=([300,800,800,800,800,800,800,800,800,800,800,1600,1600,1600,1600,1600,1600,1600,1600,1600,1600],'cm^-1')),
    ],
    spinMultiplicity = 1,
    opticalIsomers = 1,
    ...
    thermo = NASA(polynomials=[NASAPolynomial(coeffs=[1.77917,0.0414382,-2.37861e-05,-3.04974e-09,5.24492e-12,24846,15.3372], Tmin=(100,'K'), Tmax=(982.114,'K')), NASAPolynomial(coeffs=[12.9189,0.0124341,-4.48407e-06,8.15708e-10,-5.82485e-14,21868.6,-42.2228], Tmin=(982.114,'K'), Tmax=(5000,'K'))], Tmin=(100,'K'), Tmax=(5000,'K'), E0=(205.861,'kJ/mol'), comment="""Thermo group additivity estimation: group(Sis-H2Si2) + other(R) + group(Sis-H2Si2) + other(R) + group(Sis-H2Si2) + other(R) + ring(Cyclopropane)"""),
)

species(
    label = '[SiH]1[SiH2][SiH2]1(20)',
    structure = SMILES('[SiH]1[SiH2][SiH2]1'),
    E0 = (348.328,'kJ/mol'),
    modes = [
    ...
    collisionModel = TransportData(shapeIndex=2, epsilon=(1971.36,'J/mol'), sigma=(5.118e-10,'m'), dipoleMoment=(0,'C*m'), polarizability=(0,'angstroms^3'), rotrelaxcollnum=0, comment="""Epsilon & sigma estimated with fixed Lennard Jones Parameters. This is the fallback method! Try improving transport databases!"""),
    energyTransferModel = SingleExponentialDown(alpha0=(3.5886,'kJ/mol'), T0=(300,'K'), n=0.85),
    thermo = NASA(polynomials=[NASAPolynomial(coeffs=[1.91223,0.0406164,-3.33033e-05,1.00345e-08,1.72245e-13,41974,17.8187], Tmin=(100,'K'), Tmax=(1001.01,'K')), NASAPolynomial(coeffs=[11.9312,0.0108126,-3.97463e-06,7.12615e-10,-4.97638e-14,39455.6,-33.0876], Tmin=(1001.01,'K'), Tmax=(5000,'K'))], Tmin=(100,'K'), Tmax=(5000,'K'), E0=(348.328,'kJ/mol'), comment="""Thermo group additivity estimation: group(Sis-H2Si2) + other(R) + group(Sis-H2Si2) + other(R) + group(Sis-H2Si2) + other(R) + ring(Cyclopropane) + radical(SisJ_Si2_H)"""),
)
...
reaction(
    label = 'reaction1',
    reactants = ['[SiH]1[SiH2][SiH2]1(20)', '[H](21)'],
    products = ['cSi3H6(1)'],
    transitionState = 'TS1',
...
reaction(
    label = 'reaction2',
    reactants = ['[Si]1[SiH2][SiH2]1(22)', 'H2(11)'],
    products = ['cSi3H6(1)'],
    transitionState = 'TS2',
